...
$sql="SELECT COUNT(`id`) as total FROM `im_goods` WHERE `status`=1 AND `user_id`=$user_id";
...
//限制一页只显示2个
$sql="SELECT `id`,`name`,`pic`,`des` FROM `im_goods` WHERE `status`=1 AND `user_id`=$user_id ORDER BY `id`asc,`view` desc LIMIT {$offset},{$pageSize} ";
$obj=mysqli_query($con,$sql);
$goods=Array();
